...
use crate::auth::{AuthSystem, AuthConfig, Permission, UserRole};
use crate::security_monitor::{ZhtpSecurityMonitor, SecurityEvent};
use crate::security_middleware::{SecurityMiddlewareState, SecurityMiddlewareConfig, SecurityContext, start_security_cleanup_task};
use crate::health_monitor::{ZhtpHealthMonitor, HealthMetrics};
use crate::audit::{AuditTrail, AuditEventType, AuditResult};
use crate::tls::{ZhtpTlsManager, TlsConfig};
use crate::errors::{ZhtpError, ZhtpResult};
use crate::input_validation::{InputValidator, ValidationContext};
...
#[derive(Debug)]
pub struct ZhtpApiServer {
    pub state: ApiServerState,
    pub router: Router,
}
...
impl ZhtpApiServer {
    ...
    pub async fn new(
        ...
        let node_info = Arc::new(RwLock::new(NodeInfo::default()));
    ...
    async fn build_router(
        ...
        _security_middleware_state: Arc<SecurityMiddlewareState>,
    ) -> ZhtpResult<Router> {
        // Create CORS layer
        ...
        let app = Router::new()
            ...
            .route("/api/node/consensus", get(consensus_handler))
            ...
            .route("/transactions/submit", post(submit_transaction_handler)) // Legacy endpoint
            .route("/transactions/status/:tx_hash", get(get_transaction_handler)) // Legacy endpoint
            
            // Block routes
            .route("/api/blocks", get(list_blocks_handler))
            .route("/api/blocks/:height", get(get_block_handler))
            .route("/blockchain/height", get(blockchain_height_handler)) // Legacy endpoint
            .route("/blockchain/block/:height", get(get_block_handler)) // Legacy endpoint
            ...
            .route("/api/security/status", get(security_status_handler))
            .route("/security/events", get(security_events_handler)) // Legacy endpoint
            .route("/security/metrics", get(security_metrics_handler)) // Legacy endpoint
            .route("/security/status", get(security_status_handler)) // Legacy endpoint
    ...
    pub async fn start(self) -> ZhtpResult<()> {
        ...
        if let Some(_tls_manager) = &self.state.tls_manager {
            // Note: TLS integration would require additional configuration
            // For now, start without TLS but with warning
            warn!("TLS support requires additional server configuration");
...
async fn status_handler(
    ...
    Ok(Json(serde_json::json!({
        ...
        "consensus_status": node_info.consensus_status,
    })))
}
...
async fn login_handler(
    ...
    let validation_ctx = ValidationContext {
        ...
        ip_address: security_context.client_ip.parse().ok(),
        user_id: None,
        session_id: None,
    };

    (*state.input_validator).validate_username(&request.username, &validation_ctx)?;
    (*state.input_validator).validate_password(&request.password)?;
...
async fn submit_transaction_handler(
    ...
    (*state.input_validator).validate_address(&request.from, &validation_ctx)?;
    (*state.input_validator).validate_address(&request.to, &validation_ctx)?;
...
/// Blockchain height handler (legacy)
async fn blockchain_height_handler(
    State(state): State<ApiServerState>,
) -> ZhtpResult<Json<serde_json::Value>> {
    let node_info = state.node_info.read().await;
    Ok(Json(serde_json::json!({
...
) -> ZhtpResult<Json<SecurityResponse>> {
...
pub async fn start_api_server(
    ...
    _security_monitor: Arc<ZhtpSecurityMonitor>,
    _health_monitor: Arc<ZhtpHealthMonitor>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    warn!("Using legacy start_api_server function - consider upgrading to ZhtpApiServer");
    
    let config = ApiConfig {
        listen_address: bind_addr,
        enable_tls: false, // Legacy mode without TLS
        ..Default::default()
    };
    
    let auth_config = AuthConfig::default();
    
    let server = ZhtpApiServer::new(config, auth_config, None).await?;
    server.start().await?;
...
mod tests {
    ...
    fn test_default_configs() {
        ...
        assert!(api_config.enable_tls);
        assert!(api_config.require_authentication);
        assert!(api_config.enable_audit_logging);
    }
}
